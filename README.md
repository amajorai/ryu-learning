<p align="center">
  <picture>
    ...
    <img src="./icon-light.png" alt="Learning" width="144" />
  </picture>
</p>
...
Turn chats and runs into reusable skills, gated by the approval inbox, with an experience log.
...
**App:** [Install](ryu://apps/@ryu/learning) (opens the Ryu desktop app and asks you to confirm)

**CLI:**

```bash
ryu apps add @ryu/learning
```
...
companion bundle is included at [`dist/learning.ui.html`](./dist/learning.ui.html) —
...
- **`crates/core/learning` — the `ryu-learning` capability crate (NOT in this
  package).** The learning ENGINE (sweep / PRM-score / synthesize-skill /
  reward-filtered cycle), the durable `ExperienceStore` (`experience.db`), and the
  `/api/learn/*` + `/api/experience/list` router. ZERO dependency on `apps/core`:
  every Core-owned callback is inverted through the `LearningHost` trait and the
  `~/.ryu` data dir through `init_data_dir`. Core consumes it as a **path dependency,
  compiled in-process** — which is why it lives with the other extracted
  always-in-process Core capability crates (`ryu-activity`, `ryu-predict`) rather
  than under `apps-store/*/backend`, the home for app backends that ship as their own
  out-of-process sidecar. See the adjudication below.

So this package is a **UI-tier app** in the shape of `apps-store/activity` and
`apps-store/predict`: an app face (manifest + companion) whose engine is a Core
capability crate. Nothing here builds a binary.

## The crate is extracted; the PROCESS is deliberately not split

This app has a real Rust engine but **no sidecar spec and no `http.public_mount`**,
and Core keeps serving `/api/learn/*` itself (`apps/core/src/server/learning.rs`).
That is a decision, not unfinished work — and it is why the crate was reclassified
out of `apps-store/learning/backend` into `crates/core/learning`.
...
The `[[bin]] ryu-learning` in `crates/core/learning` is therefore **dormant
forward-scaffolding**:
...
- **Dependency:** `requires.apps` → `@ryu/skills >= 1.0.0` (proposed skills are
...
- **Message action:** `learning.feedback` contributes Good response / Bad response
  toggles to assistant messages. The desktop shell invokes the same
  `learning.recordFeedback` host capability used by the learning engine.
- **Channel adapter:** supported channel runtimes can map opt-in provider emoji
  reactions to the same `up` / `down` / clear vocabulary. Telegram is the first
  adapter; its `reaction_learning` settings are documented in the public channel
  guide and default to disabled.
- **No `sidecar` block and no `http.public_mount`** — intentional, per the
  ...
  endpoints. This file is the only copy — Core compiles it in via
  `include_str!("../../../../apps-store/learning/manifest.json")`, so an edit here
  changes the built-in directly.

## Surfaces as

A companion route in the shell (label **Learning**). Proposed skills route through
the approval Inbox (`@ryu/approvals`) before they become active, keeping the
learning loop human-gated.

## Star History

<a href="https://github.com/amajorai/ryu-learning/stargazers">
  <picture>
