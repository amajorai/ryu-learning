...
//! `apps-store/finetune/backend/src/paths.rs`).
//!
//! The sidecar MUST resolve the SAME data dir Core uses so it opens the SAME
//! `experience.db`. The load-bearing rule is `RYU_DIR`-env-first: Core/Kernel
//! passes `RYU_DIR` to the sidecar at spawn, guaranteeing co-location. The
//! pointer-file read + `RYU_PROFILE` suffix are replicated for faithfulness in the
//! headless case, but env-first + default is what actually guarantees the shared
//! path.
