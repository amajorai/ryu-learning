...
// Refresh button (the desktop page polled every 30s but never flickered, because
// react-query keeps prior data during a background refetch). Only the initial load
// and a manual `refetch()` set `isFetching`; only the initial load sets
// `isLoading`. This is the spec's "15s poll + post-mutation refetch, background
// refetch that doesn't flip isLoading" for the sandbox.
